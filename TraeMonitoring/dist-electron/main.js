...
const configPath = path_1.default.join(electron_1.app.getPath('userData'), 'window-state.json');
const appConfigPath = path_1.default.join(electron_1.app.getPath('userData'), 'app-config.json');
const appProfileDir = path_1.default.join(electron_1.app.getPath('userData'), 'playwright-profile');
function saveWindowState(window) {
    if (!window)
        return;
    const bounds = window.getBounds();
    try {
        fs_1.default.writeFileSync(configPath, JSON.stringify(bounds));
    }
    catch (e) {
...
function loadAppConfig() {
    try {
        if (fs_1.default.existsSync(appConfigPath)) {
            return JSON.parse(fs_1.default.readFileSync(appConfigPath, 'utf-8'));
        }
    }
    catch {
        void 0;
    }
    const def = { refreshIntervalSeconds: 300 };
    try {
        fs_1.default.writeFileSync(appConfigPath, JSON.stringify(def));
    }
    catch {
        void 0;
    }
...
function saveAppConfig(cfg) {
    ...
    try {
        fs_1.default.writeFileSync(appConfigPath, JSON.stringify(next));
    }
    catch {
        void 0;
    }
...
function createWindow() {
    var _a, _b;
    const { width: screenWidth } = electron_1.screen.getPrimaryDisplay().workAreaSize;
    ...
    mainWindow = new electron_1.BrowserWindow({
        width: windowWidth,
        height: 420,
        x: (_a = savedBounds === null || savedBounds === void 0 ? void 0 : savedBounds.x) !== null && _a !== void 0 ? _a : defaultX,
        y: (_b = savedBounds === null || savedBounds === void 0 ? void 0 : savedBounds.y) !== null && _b !== void 0 ? _b : defaultY,
        frame: false,
        transparent: true,
        alwaysOnTop: true,
...
function createActiveWindow() {
    ...
    activeWindow = new electron_1.BrowserWindow({
        width: 1120,
        height: 300,
        frame: false,
        transparent: true,
    ...
    if (isDev) {
        ...
        const fileUrl = (0, url_1.pathToFileURL)(path_1.default.join(__dirname, '../dist/index.html')).toString() + '#active';
    ...
    activeWindow.on('resize', () => {
        const views = (activeWindow === null || activeWindow === void 0 ? void 0 : activeWindow.getBrowserViews()) || [];
...
async function withPlaywrightContext(userDataDir, headless = true) {
    try {
        const ctx = await playwright_1.chromium.launchPersistentContext(userDataDir, { headless });
        return ctx;
    }
    catch {
        void 0;
    ...
    try {
        const ctx = await playwright_1.chromium.launchPersistentContext(userDataDir, { headless, channel: 'chrome' });
        return ctx;
    }
    catch {
        void 0;
    ...
    try {
        const ctx = await playwright_1.chromium.launchPersistentContext(userDataDir, { headless, channel: 'msedge' });
        return ctx;
    }
    catch {
        void 0;
...
function extractUsageFromJson(json) {
    ...
    const j = json;
    const candidates = [];
    const tryPush = (title, cur, tot, unit = '次', type = 'package') => {
    ...
    for (const key of Object.keys(j)) {
        ...
        if (val && typeof val === 'object') {
            ...
            if ('used' in o || 'limit' in o || 'total' in o || 'current' in o) {
                tryPush(key, (_a = o.used) !== null && _a !== void 0 ? _a : o.current, (_b = o.limit) !== null && _b !== void 0 ? _b : o.total, '次', key.includes('plan') ? 'plan' : 'package');
            }
            if (Array.isArray(o.items)) {
                for (const it of o.items) {
                    if (it && typeof it === 'object') {
                        const io = it;
                        tryPush(io.title || key, (_c = io.used) !== null && _c !== void 0 ? _c : io.current, (_d = io.limit) !== null && _d !== void 0 ? _d : io.total, '次', io.type || 'package');
                    }
                }
            }
        }
    ...
    const items = candidates.map((c, idx) => {
        ...
        return ({
            ...
            title: c.title || '用量',
            type: c.type || 'package',
            current: (_a = c.current) !== null && _a !== void 0 ? _a : 0,
            total: (_b = c.total) !== null && _b !== void 0 ? _b : 0,
            unit: c.unit || '次',
        });
    });
    return {
...
async function extractUsageDataFromPage(page) {
    try {
        const result = await page.evaluate(() => {
            ...
            for (let i = 0; i < lines.length; i++) {
                ...
                for (let k = i - 1; k >= Math.max(0, i - 5); k--) {
                    const t = lines[k];
                    if (/Pro plan|Extra package|计划|套餐|礼包/i.test(t)) {
                        title = t;
                        break;
                    }
                    if (!/\d/.test(t) && t.length > 0 && t.length <= 60 && !/Left|Expire|Reset/i.test(t)) {
                        title = t;
                        break;
                    }
                }
                for (let k = i - 3; k <= Math.min(lines.length - 1, i + 3); k++) {
                    ...
                    if (rm && !resetTime)
                        resetTime = rm[1].trim();
                    if (em && !expiryTime)
                        expiryTime = em[1].trim();
                    if (/Consuming/i.test(t))
                        tag = 'Consuming';
                    if (/消费/.test(t))
                        tag = '消费';
                }
                if (!title)
                    title = 'Usage';
                const type = /Pro plan|专业计划|计划/i.test(title) ? 'plan' : 'package';
        ...
        if (!result)
            return null;
        if (result.items.length === 0 && !result.resetDate)
            return null;
        return result;
    }
    catch {
...
async function parseUsageFromPage(page) {
    try {
        let apiUsage = null;
        ...
        try {
            const fromDoc = await page.evaluate(() => {
                ...
                const next = parseJsonSafe((nextNode === null || nextNode === void 0 ? void 0 : nextNode.textContent) || '');
                const scripts = Array.from(document.querySelectorAll('script[type="application/json"]'));
                const jsons = scripts.map(s => parseJsonSafe(s.textContent || '')).filter(Boolean);
                return { next, jsons };
            });
            if (fromDoc) {
                const { next, jsons } = fromDoc;
        ...
        if (apiUsage && apiUsage.items.length > 0)
            return apiUsage;
...
async function extractActiveDaysFromPage(page) {
    try {
        const res = await page.evaluate(() => {
            const textContainer = (() => {
                ...
                for (let i = 0; i < 5 && p; i++) {
                    if (p.querySelectorAll('div,section').length > 0)
                        return p;
                    p = p.parentElement;
                }
                return null;
            ...
            if (map.size === 0) {
                ...
                const levelFromIcon = (el) => {
                    const rect = el.querySelector('svg rect');
                    const fill = (rect === null || rect === void 0 ? void 0 : rect.getAttribute('fill')) || '';
                    const op = Number((rect === null || rect === void 0 ? void 0 : rect.getAttribute('fill-opacity')) || (rect === null || rect === void 0 ? void 0 : rect.getAttribute('opacity')) || '1');
                ...
                for (let ri = 0; ri < uniqRows.length; ri++) {
                    ...
                    const cells = Array.from(rowEl.querySelectorAll('[class*="calendarDay"],div,span'));
                    ...
                    for (const cell of cells) {
                        ...
                        map.set(date, { date, level: lv, count: prev === null || prev === void 0 ? void 0 : prev.count });
                    }
                }
            }
            ...
            const gridHtml = gridEl ? gridEl.outerHTML : '';
            return { cells: Array.from(map.values()), gridHtml };
        });
        if (res && Array.isArray(res.cells) && res.cells.length > 0)
            return { title: '活跃看板', months: [], cells: res.cells, gridHtml: res.gridHtml };
        if (res && typeof res.gridHtml === 'string' && res.gridHtml.length > 0)
            return { title: '活跃看板', months: [], cells: [], gridHtml: res.gridHtml };
        return null;
    }
    catch {
        return null;
...
async function fetchActiveDays() {
    try {
        ...
        let hidden = null;
        try {
            hidden = new electron_1.BrowserWindow({ show: false, webPreferences: { offscreen: false, backgroundThrottling: false } });
            ...
            const cellsFromNext = (() => {
                try {
                    const j = JSON.parse(nextTxt);
                    return extractActiveCellsFromJson(j);
                }
                catch {
                    return [];
                }
            })();
            if (cellsFromNext.length > 0)
                return { title: '活跃看板', months: [], cells: cellsFromNext };
            ...
            for (const txt of scripts) {
                try {
                    ...
                    if (cells.length > 0)
                        return { title: '活跃看板', months: [], cells };
                }
                catch { /* noop */ }
            }
            // Try SVG rects
            ...
            if (cellsSvg.length > 0)
                return { title: '活跃看板', months: [], cells: cellsSvg };
        ...
        return el ? el.outerHTML : '';
            ...
            if (typeof gridHtmlOnly === 'string' && gridHtmlOnly.length > 0)
                return { title: '活跃看板', months: [], cells: [], gridHtml: gridHtmlOnly };
            return null;
        }
        finally {
            try {
                hidden === null || hidden === void 0 ? void 0 : hidden.destroy();
            }
            catch { /* noop */ }
        }
    }
...
function extractActiveCellsFromJson(json) {
    const arr = [];
    ...
    const pushCell = (date, level, count) => {
        const d = normalizeDate(date);
        const lv = Number(level);
        const ct2 = count !== undefined ? Number(count) : undefined;
        if (d && (Number.isFinite(lv) || Number.isFinite(ct2))) {
    ...
    const walk = (obj) => {
        ...
        if (Array.isArray(obj)) {
            if (obj.length === 2 && typeof obj[0] !== 'object' && typeof obj[1] === 'number') {
                pushCell(obj[0], undefined, obj[1]);
                return;
            }
            obj.forEach(walk);
            return;
        ...
        if (typeof obj === 'object') {
            const o = obj;
            const date = (_d = (_c = (_b = (_a = o.date) !== null && _a !== void 0 ? _a : o.day) !== null && _b !== void 0 ? _b : o.dt) !== null && _c !== void 0 ? _c : o.timestamp) !== null && _d !== void 0 ? _d : o.createdAt;
            const level = (_g = (_f = o.level) !== null && _f !== void 0 ? _f : o.intensity) !== null && _g !== void 0 ? _g : o.value;
            const count = (_j = (_h = o.count) !== null && _h !== void 0 ? _h : o.times) !== null && _j !== void 0 ? _j : o.frequency;
...
async function parseUsageFromHtml(html) {
    const $ = cheerio_1.default.load(html);
    const raw = $('body').text();
    const lines = raw.split('\n').map(s => s.trim()).filter(Boolean);
    const items = [];
...
async function fetchTraeUsage() {
    const cookie = process.env.TRAE_COOKIE;
    const url = 'https://www.trae.ai/account-setting#usage';
    try {
        if (!fs_1.default.existsSync(appProfileDir)) {
            try {
                await promises_1.default.mkdir(appProfileDir, { recursive: true });
            }
            catch {
                void 0;
            }
        ...
        const hidden = new electron_1.BrowserWindow({ show: false, webPreferences: { offscreen: true, backgroundThrottling: true } });
        await hidden.loadURL(url);
        await hidden.webContents.executeJavaScript('document.readyState');
        const html = await hidden.webContents.executeJavaScript('document.documentElement.outerHTML');
...
electron_1.ipcMain.handle('update-refresh-interval', async (_e, seconds) => {
...
electron_1.ipcMain.handle('set-window-size', async (_e, size) => {
    if (!mainWindow)
        return false;
    const work = electron_1.screen.getPrimaryDisplay().workArea;
    const b = mainWindow.getBounds();
...
electron_1.ipcMain.on('set-window-position', (_e, pos) => {
    if (!mainWindow)
        return;
    const work = electron_1.screen.getPrimaryDisplay().workArea;
    const b = mainWindow.getBounds();
...
electron_1.ipcMain.on('set-resizing', (_e, flag) => {
    isResizing = !!flag;
...
electron_1.ipcMain.handle('reset-login', async () => {
    try {
        if (!fs_1.default.existsSync(appProfileDir)) {
            try {
                await promises_1.default.mkdir(appProfileDir, { recursive: true });
        ...
        while (Date.now() - startTime < timeout) {
            ...
            if (data && data.items.length > 0) {
                ...
                lastUpdate = Date.now();
                if (mainWindow)
                    mainWindow.webContents.send('usage-data-update', currentUsage);
                if (!act) {
                    try {
                        await page.goto('https://www.trae.ai/account-setting#profile', { waitUntil: 'domcontentloaded' });
                        try {
...
electron_1.ipcMain.on('open-external', (_e, url) => {
    try {
        electron_1.shell.openExternal(url);
    }
    catch { /* noop */ }
});
electron_1.ipcMain.handle('refresh-now', async () => {
    if (isRefreshing || isResizing)
        return currentUsage;
    const now = Date.now();
    const minGap = 5000;
    if (currentUsage && now - lastUpdate < minGap)
        return currentUsage;
    isRefreshing = true;
    const data = await fetchTraeUsage();
    if (data) {
...
electron_1.ipcMain.handle('get-active-days', async () => {
    const now = Date.now();
...
electron_1.ipcMain.handle('refresh-active-days', async () => {
    currentActive = null;
    lastActiveUpdate = 0;
    const data = await fetchActiveDays();
    if (data) {
...
electron_1.ipcMain.handle('open-active-window', async () => {
    createActiveWindow();
    return true;
});
electron_1.ipcMain.handle('show-live-calendar', async () => {
    try {
        if (!activeWindow)
            return false;
        ...
        let view = views[0];
        if (!view) {
            view = new electron_1.BrowserView({ webPreferences: { nodeIntegration: false, contextIsolation: true } });
...
electron_1.ipcMain.handle('get-login-status', async () => {
